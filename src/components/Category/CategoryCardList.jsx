...
const CategoryCardList = ({
  ...
  return (
    <NavLink to={`/Recipe/${categoryName === "Dessert" ? "Dessart" : categoryName.replace(/\s+/g, "-")}`} className={"w-full 2xl:w-[49.6%] lg:w-[49.3%] h-fit"}>
      <motion.li
        initial={{ opacity: 0, scale: 0.95 }}
        animate={{ opacity: 1, scale: 1 }}
        exit={{ opacity: 0, scale: 0.9, transition: { duration: 0.3 } }}
        className={`${categoryBg} cursor-pointer group h-full rounded-md sm:rounded-xl w-full flex items-center justify-between p-2`}
      >
